...
deployment_id=$(curl -H "Authorization: Token $TOKEN" -H "Accept: application/json" -H "Content-type: application/json" -X POST -d '{"ref": "bas-patch-2,"description": "Deploying branch to neverland", "environment": "development"}' https://octodemo.com/api/v3/repos/"'"$TRAVIS_REPO_SLUG"'"/deployments | jq '.id')
